  ...
  return (
    <GlassCard className="p-5">
      <div className="flex items-start justify-between">
        <div>
          <p className={`text-xs font-medium uppercase tracking-wide ${isDark ? 'text-white/40' : 'text-slate-400'}`}>{label}</p>
          <p className={`text-2xl font-bold mt-1.5 ${isDark ? 'text-white' : 'text-slate-900'}`}>{value}</p>
          {change !== undefined && (
            <p className={`text-xs mt-1 ${Number(change) >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
...
function TaskItem({ task, onComplete }) {
  ...
  return (
    <div className={`flex items-center gap-3 p-3 rounded-xl transition-colors ${isDark ? 'hover:bg-white/4' : 'hover:bg-black/3'}`}>
      <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 ${isOverdue ? 'bg-red-500/15' : 'bg-blue-500/15'}`}>
        <Icon size={13} className={isOverdue ? 'text-red-400' : 'text-blue-400'} />
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <GlassCard className="col-span-1 lg:col-span-2 p-5">
          <h3 className={`text-sm font-semibold mb-4 ${isDark ? 'text-white/80' : 'text-slate-700'}`}>Weekly Outreach Activity</h3>
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={analytics?.weeks || []} barSize={20}>
              <XAxis dataKey="label" tick={{ fill: isDark ? 'rgba(255,255,255,0.3)' : '#94a3b8', fontSize: 11 }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fill: isDark ? 'rgba(255,255,255,0.3)' : '#94a3b8', fontSize: 11 }} axisLine={false} tickLine={false} />
              <Tooltip
        ...
        <GlassCard className="p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className={`text-sm font-semibold ${isDark ? 'text-white/80' : 'text-slate-700'}`}>Today's Follow-ups</h3>
            <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${isDark ? 'bg-blue-500/20 text-blue-400' : 'bg-blue-50 text-blue-600'}`}>
              {todayTasks?.length || 0}
            </span>
          </div>
          {todayTasks?.length === 0 ? (
          ...
          <Button variant="ghost" size="sm" className="w-full mt-3 justify-center" onClick={() => navigate('/tasks')}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* Outreach by type */}
        <GlassCard className="p-5">
          <h3 className={`text-sm font-semibold mb-4 ${isDark ? 'text-white/80' : 'text-slate-700'}`}>Outreach Breakdown</h3>
          <div className="space-y-2.5">
            {outreachData.map((o, i) => (
              <div key={i} className="flex items-center gap-3">
                <span className={`text-xs w-14 ${isDark ? 'text-white/50' : 'text-slate-500'}`}>{o.name}</span>
                <div className={`flex-1 h-1.5 rounded-full overflow-hidden ${isDark ? 'bg-white/6' : 'bg-black/6'}`}>
                  <motion.div
                    initial={{ width: 0 }}
        ...
        <GlassCard className="col-span-1 lg:col-span-2 p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className={`text-sm font-semibold ${isDark ? 'text-white/80' : 'text-slate-700'}`}>Performance by Niche</h3>
            <Button variant="ghost" size="xs" onClick={() => navigate('/niches')}>View all</Button>
          </div>
          {analytics?.nicheStats?.length === 0 ? (
            <p className={`text-sm text-center py-6 ${isDark ? 'text-white/30' : 'text-slate-400'}`}>No niche data yet. <button onClick={() => navigate('/niches')} className="text-blue-400 underline cursor-pointer">Create niches</button></p>
          ) : (
                ...
                <thead>
                  <tr>
                    {['Niche','Prospects','Clients','Rate','MRR'].map(h => (
                      <th key={h} className={`text-xs font-medium text-left pb-2 pr-4 ${isDark ? 'text-white/35' : 'text-slate-400'}`}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {analytics?.nicheStats?.map((n, i) => (
                    <tr key={i} className="table-row-hover">
                      ...
                      <td className={`pr-4 py-2 ${isDark ? 'text-white/60' : 'text-slate-600'}`}>{n.prospects}</td>
                      <td className={`pr-4 py-2 ${isDark ? 'text-white/60' : 'text-slate-600'}`}>{n.clients || 0}</td>
                      <td className="pr-4 py-2">
                        <span className="text-xs font-medium text-emerald-400">
                          {n.prospects > 0 ? ((n.clients / n.prospects) * 100).toFixed(0) : 0}%
                        </span>
                      </td>
                      <td className={`py-2 ${isDark ? 'text-white/60' : 'text-slate-600'}`}>
      ...
      {/* Conversion Funnel */}
      <GlassCard className="p-5">
        <h3 className={`text-sm font-semibold mb-4 ${isDark ? 'text-white/80' : 'text-slate-700'}`}>Pipeline Funnel</h3>
        <div className="flex items-end gap-3 overflow-x-auto pb-2">
          {analytics?.funnel?.map((f, i) => {
            const max = Math.max(...(analytics.funnel.map(x => x.count)), 1)
            const pct = Math.max(12, (f.count / max) * 100)
            return (
              <div key={f.status} className="flex flex-col items-center gap-2 flex-1 min-w-[60px]">
                <span className={`text-xs font-bold ${isDark ? 'text-white/80' : 'text-slate-700'}`}>{f.count}</span>
                <motion.div
                  initial={{ height: 0 }}
                  animate={{ height: `${pct}px` }}
                  transition={{ duration: 0.5, delay: i * 0.05 }}
                  className="w-full rounded-t-lg"
                  style={{ background: FUNNEL_COLORS[i % FUNNEL_COLORS.length] + '80', minHeight: 12 }}
                />
                <span className={`text-xs text-center ${isDark ? 'text-white/40' : 'text-slate-500'}`} style={{ fontSize: 10 }}>
